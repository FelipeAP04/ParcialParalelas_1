...
static void blur_filas(const unsigned char *entrada,
    ...
    for (int i = fila_inicio; i < fila_fin; ++i) {
        ...
        /* Ventana inicial para la columna 0: columnas 0 y 1 si existen. */
...
int main(int argc, char **argv) {
    ...
    blur_filas(entrada, salida, ancho, alto, 0, alto);
    double fin = omp_get_wtime();

    printf("Blur secuencial con ventana deslizante\n");
    printf("Imagen: %d x %d\n", ancho, alto);
    printf("Tiempo: %.6f s\n", fin - inicio);
